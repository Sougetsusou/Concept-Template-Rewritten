...
不要把 category 专属布局公式直接提升到 `code.geometry`。如果发现某个
...
`assembly.py` 提供 `ConceptTemplate` 和 `combine_components()`，用于把多个 runtime
primitive 合并成一个 concept mesh。category 代码应直接从本包导入：

```python
from code.geometry import ConceptTemplate, Cylinder, Ring
```
