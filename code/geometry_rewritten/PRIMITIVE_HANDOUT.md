# geometry_rewritten Primitive Handout

本文是 `code/geometry_rewritten/` 的操作速查。category 重写时应先用这里列出的 primitive、anchor 和 transform 能力表达装配关系；只有这里缺少通用能力时，才考虑新增 primitive anchor 或 primitive/base transform API。
...
- 把单个 category 的私有布局公式沉淀进 `code.geometry_rewritten`。
