...
因此发行包中的 `rewritten_parameters/` 和 `rewritten_parameters.pkl` 已经是可直接消费的数据资产：
...
新增并统一使用 `code/geometry_rewritten/`：
...
每个 `code/code_rewritten/<Category>/concept_template.py` 都围绕 rewritten primitive 和 anchor 装配重写：
