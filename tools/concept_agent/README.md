...
这个目录只保留 runtime-only 工具。

## visualize.py

从原生参数包实例化 concept，合并 object mesh，并可选择打开 Open3D viewer 或保存 OBJ/PNG。
...
- `--category <Category>`: 从 `code/concepts/<Category>/parameters/` 加载整包。
- `--params <path>`: 从目录包、`index.json`、单个 `object_XXXX.json` 或 `parameters.pkl` 加载参数。
...
- `native_params.py`: 参数包读取、category module 加载、concept 实例化和 mesh 合并。
