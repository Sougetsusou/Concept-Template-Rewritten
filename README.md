# Concept Template Rewritten Runtime
...
```text
code/
  geometry_rewritten/          # rewritten primitives、transform 和装配基础设施
  code_rewritten/
    <Category>/
      __init__.py
      concept_template.py      # rewritten concept constructors
      rewritten_parameters/    # object-level JSON 参数包
      rewritten_parameters.pkl # 与 JSON 参数包等价的整包 pickle
...
tools/
  concept_agent/
    ...
    lib/
      native_params.py         # rewritten 参数读取、实例化、mesh 合并
...
```text
reports/<Category>_rewritten_native_visualization/
```

## 参数包格式

- `rewritten_parameters/` 是 object-level JSON 目录包。
- `index.json` 保存 category 摘要和 object 索引。
- 每个 `object_XXXX.json` 只保存一个完整 object，适合单独审阅、diff 和可视化。
- `rewritten_parameters.pkl` 与 JSON 目录包内容等价，适合一次性加载整包。
- 每个 concept 的 `template` 是 rewritten class 名，`parameters` 是该 class constructor 的原生参数。
...
package = load_rewritten_parameter_package(
    "code/code_rewritten/Bottle/rewritten_parameters/object_0010.json"
)
category_module = load_rewritten_category_module(package["category"])
...
- 本包不提供 legacy-to-native 参数转换；发行包内的参数已经是 rewritten constructor 原生参数。
- 不应手工编辑 `rewritten_parameters/` 或 `rewritten_parameters.pkl`，除非重新生成并验证整套参数资产。
